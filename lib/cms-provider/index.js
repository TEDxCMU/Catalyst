...
async function fetchCmsMasterRef() {
  ...
  const masterVersion =
    json.refs?.find((apiVersion) => apiVersion.id === 'master') || null;
...
export async function getStages() {
  ...
  const reformattedData = data.allExperiences.edges[0].node.body[3].fields.map(
    ({ stage }) => {
      return {
        name: stage.name,
...
export async function getSchedule() {
  ...
  `);
  const reformattedData = data.allExperiences.edges[0].node.body[1].fields.map(
    ({ event }) => {
      return {
        title: event.title,
...
export async function getSpeakers() {
  ...
  const reformattedData = data.allExperiences.edges[0].node.body[0].fields.map(
    ({ speaker }) => {
      return {
        ...
        image: {
          url:
            speaker.image?.url.replace('compress,format', 'format') ||
...
export async function getInnovators() {
  ...
  const reformattedData = data.allExperiences.edges[0].node.body[2].fields.map(
    ({ innovator }) => {
      return {
        ...
        image: {
          url:
            innovator.companyLogo?.url.replace('compress,format', 'format') ||
            'https://images.prismic.io',
        ...
        people: innovator.innovators.map((innovator) => ({
          ...
          image: {
            url:
              innovator.image?.url.replace('compress,format', 'format') ||
              'https://images.prismic.io',
