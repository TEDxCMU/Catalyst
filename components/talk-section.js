  ...
  return (
    <Modal active={active} setActive={setActive} large>
        ...
        <div className={styles.content}>
          <p className={styles.time}>{formatDate(talk.startTime).toUpperCase()} – {formatDate(talk.endTime).toUpperCase()}</p>
          <h2 className={styles.title}>{talk.title}</h2>
          <div className={styles.people}>
            {talk.speaker ?(
                <div className={styles.person} key={talk.speaker.name}>
                    <img className={styles.avatar} src={talk.speaker.image?.url} alt={talk.speaker.image?.alt} />
                    <div>
                    <p className={styles.name}>{talk.speaker?.name.toUpperCase()}</p>
